...
export default function Reader({books,initial,back}:{books:Book[];initial?:{bookId:string;chapter:number;from:number;to:number};back?:()=>void}){
 ...
 function openLocation(nextBook:string,nextChapter:number,nextVerse?:number){setBookId(nextBook);setChapter(nextChapter);setTargetVerse(nextVerse??null);setQuery('');setResults([]);if(nextVerse===undefined)window.scrollTo({top:0,behavior:'smooth'})}
